...
class NABWebApiBaseSdkExtensionTest extends KernelTestCase
{
    ...
    public function testExtensionWillSetExpectedService(
        ...
        // Add 1 to the count for the Container Interface Service Definition from Symfony.
        $this->assertCount(count($clients) + 1, $containerBuilder->getDefinitions());

        foreach ($clients as $client) {
            // Assert we still have a service definition.
            $definition = $containerBuilder->getDefinition(
                sprintf('nab.web_api_sdk.%s.%s_client', $client, $serviceClassification)
            );
            ...
            if ($expectedConstructorDefinitions) {
                foreach ($expectedConstructorDefinitions as $index => $className) {
                    ...
                    $this->assertSame($argumentDefinition->getClass(), $className);
                }
            }

            // Almost there, assert the api key was set
            $this->assertSame('abc', $definition->getArgument($expectedXApiKeyArgumentIndex));
            ...
            $this->assertSame($expectedGuzzleConfig, $guzzleConfiguration);
        }
    }
