...
class NABWebApiBaseSdkExtension extends Extension
{
    ...
    /**
     * Override this function in any bundle that extends this base bundle. Failure to do so will result in you
     * not being able to extend this configuration or get the proper root key name that is required to process
     * your bundle's configuration (Symfony error - could not find an extension to load X configuration).
     *
     ...
     * @param Definition $unwrappedDefinition
     * @param string $serviceKey
     * @param string|null $wrappedByConfig
     */
    private function wrapDefinition(
        ...
        if ($wrappedByConfig) {
            if (class_exists($wrappedByConfig)) {
                ...
                throw new \InvalidArgumentException("Cannot wrap service {$serviceKey} in class {$wrappedByConfig}.");
            }
        } else {
            $containerBuilder->setDefinition($serviceKey, $unwrappedDefinition);
        }
    }
}
